...
public class Mentions extends Model {
    ...
    public Mentions(JSONObject object) {
        super();
        ...
        try {
            ...
            if (object.has("retweeted_status")) {
                ...
                this.retweet_user = object.getJSONObject("user").getString("screen_name");
    ...
    public static ArrayList<Mentions> fromJSON(JSONArray jsonArray) {
        ArrayList<Mentions> tweets = new ArrayList<Mentions>(jsonArray.length());
    ...
    public static Mentions findOrCreateFromJson(JSONObject json) {
        Long rId = null; // get just the remote id
        try {
            rId = json.getLong("id");

            Mentions existingUser =
                    new Select().from(Mentions.class).where("tweetid = ?", rId).executeSingle();
            if (existingUser != null) {
                // found and return existing
                return existingUser;
            } else {
                // create and return new user
                Mentions tweet = new Mentions(json);
                tweet.save();
                return tweet;
            }
        } catch (JSONException e) {
    ...
    public static Cursor fetchResultCursor() {
        From query = new Select()
                ...
                .on("Mentions.User=Users.Id");

        Cursor resultCursor = Cache.openDatabase().rawQuery(query.toSql(), query.getArguments());
        return resultCursor;
    }

    public static Mentions getTweet(Long tweetid) {
        Mentions existingUser = new Select().from(Mentions.class).where("tweetid = ?", tweetid).executeSingle();
        if (existingUser != null) {
            // found and return existing
            return existingUser;
        }
